"""Robust optimization baseline using lower-tail CVaR over scenarios."""
...
class RobustCVaRPlanner:
    """Scenario-based discrete planner optimizing tail-average profit."""
    ...
    def _cvar(self, scores: list[float]) -> float:
        sorted_scores = sorted(scores)
        tail_count = max(1, len(sorted_scores) // 4)
        return float(mean(sorted_scores[:tail_count]))
...
def run_baseline(
    ...
    planner = RobustCVaRPlanner(env_config, agents, action_dim, horizon, blocks, rng)
    plan, train_scores = planner.optimize(scenario_seeds, candidates)
    return make_table_policy(plan, agents), train_scores


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate the robust optimization CVaR baseline.")
    parser.add_argument("--config", default="config.py")
    parser.add_argument("--uncertainty_profile", default="moderate", choices=("none", "moderate", "stress"))
    parser.add_argument("--robust_training_profile", default="stress", choices=("moderate", "stress"))
    ...
    parser.add_argument("--output_dir", default="results/baselines/robust_optimization_cvar")
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    eval_env_config = build_env_config(args.config, args.uncertainty_profile)
    config_module = load_config_module(args.config)
    base_env_config = ConfigAdapter(config_module).build_env_config()
    train_env_config = apply_uncertainty_profile(base_env_config, args.robust_training_profile)
    env = RefinerySchedulingEnv(eval_env_config, seed=args.seed)
    ...
    rows, methods = evaluate_methods(eval_env_config, {"robust_optimization_cvar": policy}, eval_seeds)
    ...
    with open(os.path.join(args.output_dir, "comparison_summary.json"), "w", encoding="utf-8") as fh:
        json.dump(
            {
                "robust_training_profile": args.robust_training_profile,
                "planner_train_scores": train_scores,
                "methods": methods,
            },
            fh,
            ensure_ascii=False,
